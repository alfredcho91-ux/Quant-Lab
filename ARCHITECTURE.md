...
| `deepcoin` | 읽기 전용 체결·종료 포지션 동기화, TPSL 복기, 거래 시점 지표 스냅샷 |
...
1. `JournalPage`는 `GET /api/deepcoin/status`로 서버의 환경 변수 설정 여부만 확인하고, API secret을 브라우저로 전송하지 않습니다.
...
3. `modules/deepcoin/snapshot.py`는 새 이벤트별로 Binance Spot OHLCV를 해당 시각까지 로드하고, **이벤트 전에 완료된 마지막 봉**만 사용합니다. `service.py`는 동기화와 저장 오케스트레이션을 담당하며, 저장소는 외부 ID를 한 번에 조회한 뒤 신규 삽입·기존 종료 포지션 갱신을 각각 단일 트랜잭션으로 처리합니다.
...
6. 종료 거래의 통합 리포트는 `/api/indicators/trade-report/{coin}/{interval}`에서 표시용 캔들·모멘텀 시계열을 받습니다. VPVR와 VWAP는 사용자가 선택한 진입 또는 종료 시각보다 먼저 끝난 확정봉 300개를 기준으로 계산하고, 종료 이후 흐름 확인용 캔들은 기준값 계산에서 제외합니다.
...
8. `TradeAnalysisPage`는 손익 대시보드가 아니라 승패 조건과 매매 품질 발견에 집중합니다. 종료 거래를 기간·결과·투자금 대비 순수익률 구간별로 필터링하고, 실제 최초 진입 체결의 확정봉 스냅샷만 사용해 승패 빈도와 지표 차이를 비교합니다. `GET /api/journal/excursions`는 종목별 15분봉을 묶어 조회해 거래 생존 구간 내부의 MFE/MAE를 계산합니다.
...
11. `StopLossExpectationTool`은 별도 API를 호출하지 않고 이미 조회한 종료 거래와 15분봉 MFE/MAE 결과를 재사용합니다. 순수 계산은 `features/tradeAnalysis/stopLossExpectation.ts`에 격리합니다. 투자금 기준에서는 MAE를 저장된 레버리지로 환산해 N% 도달 여부를 판정하고, 발동 거래를 `-N% - 실제 수수료율`, 미발동 거래를 저장된 실제 순수익률로 계산합니다. 가격 기준은 비용 전 가격 움직임을 비교합니다. 레버리지나 필수 경로가 없는 거래는 임의 보정하지 않습니다.
12. `GET /api/journal/sl-tp-analysis`는 `sl_tp_analysis.py`에서 거래별 Binance Spot 5분봉을 최초 진입부터 실제 종료까지 재생합니다. SL/TP별 최초 도달 봉을 거래당 한 번 계산해 최대 800개 조합에서 재사용하고, 같은 5분봉 동시 도달은 ambiguous SL로 보수 처리합니다. 전체 결과와 과거 70% 선택·최근 30% 검증을 함께 반환하며 프런트의 `SlTpExpectationAnalysis`가 추천 범위, 단일 후보, 실제 청산 비교, Heatmap과 전체 표를 표시합니다. 경로 캐시와 조합 결과 캐시는 분리해 입력 범위 변경 시 시장 데이터를 다시 받지 않습니다.
...
Deepcoin 체결가는 거래소 체결 데이터이고 지표와 VPVR는 Binance Spot kline 기반입니다. 이 둘은 같은 시각에도 가격과 거래량이 완전히 일치하지 않을 수 있습니다.
