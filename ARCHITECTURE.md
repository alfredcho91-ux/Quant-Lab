...
4. `core/indicator_pipelines.py`의 추세판단 계산을 재사용해 1h/2h/4h/1d RSI, MACD, Slow Stochastic, Stoch RSI를 계산합니다. 각 시간대의 240봉(일봉 180봉) VPVR도 계산합니다. Entry RVOL20은 진입 직전 완료봉 거래량을 그 이전 20개 완료봉 평균으로 나눈 관찰값이며, 진행 중 봉의 최종 거래량은 사용하지 않습니다.
...
8. `PlanLabPage`는 과거 종료 거래의 계획 입력과 공식 Plan Lab 분석을 분리합니다. TP2가 입력된 계획은 `TP1 50% + TP2 잔여 50%` 규칙으로 저장·분석하고, TP2가 없으면 TP1 100% 단일 청산을 유지합니다. 입력 폼의 `Target R:R` 미리보기는 진입·손절·TP1·선택 TP2 가격만으로 즉시 계산하는 프런트엔드 보조값이며, 회고 거래에서는 실제 Entry를 참고하되 계획 Entry로 저장하지 않습니다. 이 미리보기는 공식 Plan R, Plan PnL, Delta를 대체하지 않습니다. Split Plan의 청산 후 상태를 복원할 수 없는 거래는 TP1-only로 대체하지 않고 해당 사후 분석 표본에서 제외해 coverage에 표시합니다.
