...
**WolGem Quant Master Backend** - 암호화폐 트레이딩 분석을 위한 FastAPI 백엔드
...
- **위치**: `backend/strategy/streak/common.py` - `wilson_confidence_interval()`
- **공식**: $z = 1.96$ (95% 신뢰수준) 기준
- **금지**: 공식 변경, $z = 1.96$ 변경, 'Low Reliability' 로직 제거 금지

### 4. Bonferroni 보정
- **위치**: `backend/strategy/streak/common.py` - `analyze_interval_statistics()`
- **공식**: $\alpha_{adjusted} = \frac{\alpha}{n_{comparisons}}$
- **금지**: 보정 로직 제거 금지

### 5. EST 시간대 변환
- **위치**: `backend/strategy/streak/common.py` - `calculate_intraday_distribution()`
...
├── routes/                 # API 라우터 (얇은 레이어)
│   ├── streak.py          # 연속 봉패턴 분석 API
│   ├── stats.py           # 통계 분석 API (BB Mid, Combo Filter, Hybrid 등)
...
### 주간 패턴 분석
- **POST** `/api/weekly-pattern`
- **로직**: `backend/strategy/weekly_pattern/` 모듈
...
- **공통 함수**: `compute_live_indicators()` - EMA, MACD, RSI, ADX 계산
