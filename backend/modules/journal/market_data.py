...
def load_journal_ohlcv(
    ...
    *,
    ...
    ``exchange`` remains accepted only so existing analysis callers keep their
    contract while all exchange-specific candle branches stay disabled.
    """
    _ = exchange
    frame = fetch_binance_klines(symbol.replace("/", ""), timeframe, total_candles, end_time)
