    ...
    *,
    ...
    with _connect(db_path) as conn:
        ...
        if existing.get("plan_recorded_at") and "plan_recorded_at" in payload:
            # Keep the first recorded plan time so later tag edits cannot turn a
            # previously in-trade plan into a misleading post-exit plan.
...
def create_behavior_rule(
    rule: Dict[str, Any], *, db_path: Optional[Path] = None
) -> Dict[str, Any]:
...
def update_behavior_rule(
    rule_id: int, rule: Dict[str, Any], *, db_path: Optional[Path] = None
) -> Optional[Dict[str, Any]]:
    fields: Dict[str, Any] = {}
    if "name" in rule:
