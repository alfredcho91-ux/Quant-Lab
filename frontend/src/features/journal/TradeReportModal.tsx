...
function pathRequestConfig(entryMs: number, exitMs: number): { interval: TradePathInterval; limit: number } | null {
  ...
  for (const interval of ['5m', '15m'] as const) {
    const intervalMs = PATH_INTERVAL_MS[interval];
    const durationBars = Math.ceil(duration / intervalMs);
    if (durationBars <= MAX_PATH_CANDLES) {
      return { interval, limit: Math.max(100, durationBars + 12) };
    }
  }
  return null;
}
...
export default function TradeReportModal({
  ...
  const pathQuery = useQuery({
    queryKey: [
      ...
      Number.isFinite(exitMs) ? exitMs : null,
      entry.entry_price,
      entry.exit_price,
      entry.direction,
    ],
    queryFn: () => getTradeReport(coin as string, pathConfig!.interval, {
      ...
      end_time: (exitMs as number) + PATH_INTERVAL_MS[pathConfig!.interval] * 2,
      as_of: entryMs as number,
      profile_candles: 100,
    }),
    enabled: Boolean(
      ...
      && coin
      && pathConfig
      && entry.direction && (entry.direction === 'Long' || entry.direction === 'Short')
  ...
  const pathSummary = useMemo(() => {
    if (
      pathConfig == null
      || pathQuery.data == null
      || resolvedEntryTime.datetime == null
      || entry.datetime == null
  ...
  const pathEvents = useMemo<TradePathChartMarker[]>(() => {
    if (pathSummary == null || entry.direction == null) return [];
    ...
    return [
      pathSummary.favorable_peak == null ? null : {
        datetime: new Date(pathSummary.favorable_peak.time).toISOString(),
        price: pathSummary.favorable_peak.price,
        label: tradePathMarkerLabel(pathSummary.favorable_peak, isKo),
        position: favorablePosition,
        color: '#a78bfa',
      },
      pathSummary.entry_retest == null ? null : {
        datetime: new Date(pathSummary.entry_retest.time).toISOString(),
        price: pathSummary.entry_retest.price,
        label: tradePathMarkerLabel(pathSummary.entry_retest, isKo),
        position: 'inBar',
        color: '#fbbf24',
      },
      pathSummary.adverse_peak_after_retest == null ? null : {
        datetime: new Date(pathSummary.adverse_peak_after_retest.time).toISOString(),
        price: pathSummary.adverse_peak_after_retest.price,
        label: tradePathMarkerLabel(pathSummary.adverse_peak_after_retest, isKo),
        position: adversePosition,
        color: '#f97316',
      },
      ...
      <div className="flex h-[94vh] w-full max-w-[1500px] flex-col overflow-hidden border border-dark-700 bg-dark-900 shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {isClosedPosition(entry) && (
            <div className="mb-5 border-y border-dark-700 py-3">
              <div className="text-[11px] font-semibold text-dark-500">
                {isKo ? '보유 중 가격 흐름' : 'Price Path While Held'}
              </div>
              {pathQuery.isLoading ? (
                <div className="mt-1 flex items-center gap-2 text-xs text-dark-500">
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  {isKo ? '실제 보유 구간의 가격 흐름을 계산 중' : 'Calculating the price path during the position'}
                </div>
              ) : pathSummary ? (
                <>
                  <div className="mt-1 text-sm font-semibold leading-6 text-dark-200">
                    {tradePathSummaryText(pathSummary, isKo)}
                  </div>
                  <div className="mt-1 text-[10px] text-dark-500">
                    {pathSummary.interval} {isKo ? '봉의 실제 보유 구간만 사용 · 봉 고가/저가 기준 · 수수료·펀딩 제외' : 'bars fully inside the holding period · high/low basis · excludes fees and funding'}
                    {pathQuery.data?.source ? ` · ${pathQuery.data.source}` : ''}
                  </div>
                </>
              ) : (
                <div className="mt-1 text-xs text-dark-500">
                  {pathConfig == null
                    ? isKo ? '보유 시간이 길거나 진입·청산 시간이 없어 가격 흐름을 계산하지 못했습니다.' : 'The holding period is too long or entry/exit timing is unavailable.'
                    : pathQuery.isError
                      ? isKo ? '가격 흐름 데이터를 불러오지 못했습니다.' : 'Could not load the price path data.'
                      : isKo ? '가격 흐름 분석 데이터 없음' : 'Price path analysis unavailable'}
                </div>
              )}
            </div>
          )}
