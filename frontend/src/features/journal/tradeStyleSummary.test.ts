...
function trades(count: number, overrides: Partial<AnalyzedTrade> = {}): AnalyzedTrade[] {
  return Array.from({ length: count }, (_, index) => ({
    ...
    entryDatetime: '2026-08-01T00:00:00Z',
    entryTimeConfidence: 'confirmed',
    holdingMinutes: 360,
    excursion: null,
    entrySnapshot: {
      event_type: 'fill',
      reference: 'last_completed_candle_before_fill',
      timeframes: {
        '4h': {
          status: 'complete',
          rsi: 30,
          stoch_rsi: { k: 10 },
          slow_stochastic: { '5-3-3': { k: 10 } },
          macd: { histogram: index % 2 === 0 ? 1 : -1 },
        },
      },
    },
    ...overrides,
...
function qualities(count: number, overrides: Partial<TradeQualityItem> = {}): TradeQualityItem[] {
  return Array.from({ length: count }, (_, index) => ({
    ...
    trend_states: {},
    market_regime: { id: 'mixed', alignment: 'aligned', trade_bias: 'up' },
...
describe('trade style summary', () => {
  it('asks for more data when there are no or too few completed trades', () => {
  ...
  it('classifies a consistent counter-trend, oversold, short swing period', () => {
    const result = summarizeTradeStyle(trades(6), qualities(6), true);

    expect(result.text).toBe('매매 스타일  역추세 평균회귀형 · 데이트레이딩 · 과매도 선호 · 다소 이른 청산');
  });

  it('classifies trend following and preserves a mixed long/short sample', () => {
    const result = summarizeTradeStyle(
      trades(6, { holdingMinutes: 2_000 }),
      qualities(6, { trade_alignment: 'with_trend', quality_class: 'good_entry_good_exit' }),
      true,
    );
  ...
  it('uses mixed and unclear labels when no style dominates', () => {
    const result = summarizeTradeStyle(
      trades(6),
      qualities(6).map((item, index) => ({
        ...item,
        trade_alignment: index % 2 === 0 ? 'with_trend' : 'counter_trend',
        quality_class: index % 3 === 0
