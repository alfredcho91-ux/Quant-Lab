...
function entryTrait(trades: AnalyzedTrade[], isKo: boolean): string {
  const signals = trades.flatMap((trade) => {
    ...
    const slow = snapshot.slow_stochastic?.['5-3-3']?.k;
    const values = [snapshot.rsi, snapshot.stoch_rsi?.k, slow].filter(
      (value): value is number => value != null && Number.isFinite(value),
    );
    if (values.length === 0) return [];
    const oversold = values.some((value) => value <= 20) || (snapshot.rsi != null && snapshot.rsi <= 40);
    const overbought = values.some((value) => value >= 80) || (snapshot.rsi != null && snapshot.rsi >= 60);
    const histogram = snapshot.macd?.histogram;
    const momentum = histogram != null && Number.isFinite(histogram)
      && ((trade.entry.direction === 'Long' && histogram > 0) || (trade.entry.direction === 'Short' && histogram < 0));
    return [{ oversold, overbought, momentum }];
  });
  if (signals.length < MIN_SAMPLE_SIZE) return isKo ? '뚜렷한 진입 특성 없음' : 'No clear entry trait';

  const oversold = percentage(signals.filter((signal) => signal.oversold).length, signals.length);
  const overbought = percentage(signals.filter((signal) => signal.overbought).length, signals.length);
  const momentum = percentage(signals.filter((signal) => signal.momentum).length, signals.length);
...
function exitTrait(qualities: TradeQualityItem[], isKo: boolean): string {
  const classified = qualities.filter((item) => [
    'good_entry_early_exit',
    'good_entry_late_exit',
    'good_entry_good_exit',
  ].includes(item.quality_class));
  if (classified.length < MIN_SAMPLE_SIZE) return isKo ? '청산 성향 불명확' : 'Exit tendency unclear';

  const early = percentage(classified.filter((item) => item.quality_class === 'good_entry_early_exit').length, classified.length);
  const late = percentage(classified.filter((item) => item.quality_class === 'good_entry_late_exit').length, classified.length);
  const appropriate = percentage(classified.filter((item) => item.quality_class === 'good_entry_good_exit').length, classified.length);
...
export function summarizeTradeStyle(
  ...
  const withTrend = percentage(alignments.filter((item) => item.trade_alignment === 'with_trend').length, alignments.length);
  const counterTrend = percentage(alignments.filter((item) => item.trade_alignment === 'counter_trend').length, alignments.length);
  const mainTrait = alignments.length >= MIN_SAMPLE_SIZE && counterTrend >= DOMINANT_SHARE
    ...
    : alignments.length >= MIN_SAMPLE_SIZE && withTrend >= DOMINANT_SHARE
      ...
      : (isKo ? '혼합형' : 'Mixed');
  const holdingMinutes = trades.flatMap((trade) => (
