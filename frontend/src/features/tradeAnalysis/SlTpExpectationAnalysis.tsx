      ...
      <div className="border-t border-dark-700">
        ...
        {query.isLoading ? (
          ...
          <div>
            ...
            <details className="border-b border-dark-700 p-4"><summary className="cursor-pointer text-xs font-semibold text-white">{isKo ? `SL/TP 조합별 전체 성과표 (${bundle.candidates.length})` : `All SL/TP combinations (${bundle.candidates.length})`}</summary><div className="mt-3"><CandidateTable candidates={bundle.candidates} best={best} isKo={isKo} /></div></details>

            <details className="px-4 py-3 text-[10px] leading-4 text-dark-600"><summary className="flex cursor-pointer items-center gap-1 text-dark-500"><Info className="h-3 w-3" />{isKo ? '계산 기준과 데이터 범위' : 'Method and coverage'}</summary><p className="mt-2">{isKo ? '각 거래의 최초 진입부터 실제 종료까지 완전히 포함된 거래소 우선 5분봉을 시간순으로 재생합니다. Deepcoin SWAP 캔들이 없거나 요청에 실패하면 Binance Spot 대체 데이터를 사용하며, 이 경우 시장 데이터 경고가 표시됩니다. 같은 5분봉에서 SL과 TP가 모두 닿으면 순서를 알 수 없으므로 ambiguous로 기록하고 SL 처리합니다. 결과는 방향성 가격수익률에서 해당 거래의 실제 수수료율을 차감하며, 가상 종료 시점의 펀딩과 슬리피지는 포함하지 않습니다. 누적 수익과 Max Drawdown은 매 거래에 동일 자본을 순차 재투자한 복리 가정입니다.' : 'The simulator replays exchange-native 5m candles when available and labels Binance Spot fallback use. Same-bar collisions are marked ambiguous and conservatively stopped. Recorded fee rate is used as a proxy; funding and slippage are excluded.'}</p><p className="mt-1">{isKo ? `경로 ${query.data?.coverage.analyzed_positions || 0}/${query.data?.coverage.closed_positions_considered || 0}건 · 수수료 반영 ${query.data?.coverage.fee_proxy_positions || 0}건 · 과거 70% 선택 / 최근 30% 검증` : `${query.data?.coverage.analyzed_positions || 0}/${query.data?.coverage.closed_positions_considered || 0} paths · fees ${query.data?.coverage.fee_proxy_positions || 0} · 70/30 train-validation`}</p>{(query.data?.warnings.length || 0) > 0 && <p className="mt-1 text-amber-400/80">{isKo ? `시장 데이터 경고 ${query.data?.warnings.length}건` : `${query.data?.warnings.length} market-data warnings`}</p>}</details>
          </div>
        )}
      </div>
    </details>
