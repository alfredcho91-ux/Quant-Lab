...
export default function TradeQualityAnalysis({
  ...
  return (
    <div className="space-y-4">
      ...
      {(showRegimes || showExitAnalysis) && <details className="group" open>
        <summary className="flex cursor-pointer list-none items-center justify-between border border-dark-700 bg-dark-900/30 px-4 py-3 text-sm font-medium text-dark-200 hover:text-white">
          <span>{showRegimes && showExitAnalysis ? (isKo ? '진입 환경과 청산 복기' : 'Entry Context and Exit Review') : showRegimes ? (isKo ? '진입 당시 시장 환경' : 'Entry Market Regimes') : (isKo ? '청산 복기' : 'Exit Review')}</span>
          <span className="flex items-center gap-2 text-[11px] font-normal text-dark-500">{showRegimes ? `${analysis.regimes.length}${isKo ? '개 시장 상황' : ' regimes'}` : (isKo ? '4H 종료 이후 완료봉 기준' : 'Completed 4H post-exit candles')}<ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" /></span>
        </summary>
        <div className="mt-3 space-y-3">
          {showRegimes && <section className="border border-dark-700 bg-dark-900/20 p-4">
          ...
          <div>
            <h3 className="text-xs font-medium text-dark-200">{isKo ? '조금 더 보유했다면' : 'If Held Beyond the Actual Exit'}</h3>
