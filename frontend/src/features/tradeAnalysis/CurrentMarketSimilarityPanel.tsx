      ...
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-dark-700 px-4 py-3">
        <div>
          ...
          <div className="mt-1 text-[11px] text-dark-500">
            {query.data
              ? `${query.data.symbol} · ${trendDirectionLabel(query.data.trend_states)} · ${formatDate(query.data.as_of, isKo, true)} · ${query.data.indicator_snapshot.market_source || 'Binance USDT-M Futures'}`
              : `${coin}/USDT`}
          </div>
        </div>
        <button
