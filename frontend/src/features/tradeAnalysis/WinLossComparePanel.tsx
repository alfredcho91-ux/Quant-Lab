...
type Mode = 'all' | 'wins' | 'losses' | 'compare';
...
export default function WinLossComparePanel({ rows, conditions, isKo, mode, onModeChange, timeframe, onTimeframeChange, onIndicatorOpen, onConditionOpen }: {
  ...
  return <section className="rounded-2xl border border-dark-700 bg-dark-900/35 p-5 shadow-[0_8px_30px_rgba(0,0,0,0.12)] sm:p-6">
    ...
    <div className="mt-5 flex flex-col gap-3 border-y border-dark-700 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="grid grid-cols-2 gap-1 border border-dark-700 bg-dark-950/40 p-1 sm:grid-cols-4">{modes.map((item) => <button key={item.id} type="button" onClick={() => onModeChange(item.id)} className={`min-h-9 px-3 text-xs font-medium ${mode === item.id ? 'bg-primary-500/20 text-primary-100' : 'text-dark-400 hover:text-white'}`}>{item.label}</button>)}</div>
