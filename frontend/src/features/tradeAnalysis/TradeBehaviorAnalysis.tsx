...
export function BehaviorLeakSummary({
  ...
  const leaks = data?.biggest_leaks.slice(0, 3) || [];
...
export default function TradeBehaviorAnalysis({ data, isLoading, isError, startTime, endTime, minimumAbsNetReturnPct, isKo, onRetry, onSelectTrade, showJournal = true, showComparison = true, showRules = true }: { data?: JournalBehaviorAnalysisData; isLoading: boolean; isError: boolean; startTime: number; endTime: number; minimumAbsNetReturnPct: number; isKo: boolean; onRetry: () => void; onSelectTrade: (id: number) => void; showJournal?: boolean; showComparison?: boolean; showRules?: boolean }) {
  ...
  const journalEmpty = data.plan_summary.recorded_trade_count === 0 && data.setup_stats.length === 0 && data.mistake_stats.length === 0;
  return <div className="space-y-4">
    {showJournal && (journalEmpty ? <section className="border border-dashed border-dark-700 bg-dark-900/20 p-5"><div className="text-sm font-medium text-dark-200">{isKo ? '매매일지 기록이 필요합니다' : 'Journal records are needed'}</div><p className="mt-1 text-xs text-dark-500">{isKo ? '계획 대비 실제 결과, Setup, 실수 태그를 기록하면 이 세 가지 분석을 함께 볼 수 있습니다.' : 'Record plans, setups, and mistakes to unlock these three reviews together.'}</p><a href="/journal" className="mt-3 inline-block text-xs text-primary-200 hover:text-white">{isKo ? '매매일지에서 기록 시작 →' : 'Start recording in Journal →'}</a></section> : <><PlanReview data={data} isKo={isKo} onSelectTrade={onSelectTrade} /><div className="grid gap-4 xl:grid-cols-2"><TagPerformance title={isKo ? '어떤 Setup이 잘 맞았나' : 'Setup Performance'} note={isKo ? '사용자가 기록한 태그의 연관성 통계입니다. 종료 후 입력된 태그는 인과관계를 증명하지 않습니다.' : 'This is an association from user-recorded tags; tags entered after exit do not prove causation.'} rows={data.setup_stats} toneClass="text-bull" minimumSample={data.minimum_conclusion_sample} isKo={isKo} onSelectTrade={onSelectTrade} /><TagPerformance title={isKo ? '어떤 실수가 손실을 만들었나' : 'Mistake Performance'} note={isKo ? '사후 복기 태그와 실제 결과의 연관성입니다. 원인을 단정하는 지표로 사용하지 마세요.' : 'This links post-trade review tags with results; it does not prove a causal cause.'} rows={data.mistake_stats} toneClass="text-bear" minimumSample={data.minimum_conclusion_sample} isKo={isKo} onSelectTrade={onSelectTrade} /></div></>)}
    {showComparison && <Evaluator data={data} startTime={startTime} endTime={endTime} minimumAbsNetReturnPct={minimumAbsNetReturnPct} isKo={isKo} onSelectTrade={onSelectTrade} />}
    {showRules && <RuleEditor data={data} isKo={isKo} />}
  </div>;
}
