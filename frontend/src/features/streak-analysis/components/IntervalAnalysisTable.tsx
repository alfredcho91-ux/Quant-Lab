...
export default function IntervalAnalysisTable({ result, isKo }: IntervalAnalysisTableProps) {
  ...
  const hasDisp = result.disp_by_interval && Object.keys(result.disp_by_interval).length > 0;
  ...
  return (
    <div className="card p-6">
      ...
      <p className="text-dark-400 text-sm mb-4">
        {isKo
          ? '전일 지표 기준, 다음 봉 양봉 확률. ⚠️ 다중비교 보정 적용됨'
          : 'Green candle probability by prev-day indicator. ⚠️ Bonferroni correction applied'}
      </p>
        ...
        {hasDisp && (
          <div className="bg-dark-800 rounded-xl p-4">
            <div className="text-cyan-400 font-semibold mb-3 flex items-center gap-2">
              📉 {isKo ? '전일 이격도 구간' : 'Prev Disparity Zone'}
              <span className="text-dark-500 text-xs font-normal">
                ({Object.keys(result.disp_by_interval!).length}
                {isKo ? '개 구간 테스트' : ' intervals tested'})
              </span>
            </div>
            <div className="space-y-2">
              {Object.entries(result.disp_by_interval!).map(([interval, data]) => {
                const isHighProb = data.rate >= 60 && data.sample_size >= 3;
                const isStatSig = data.is_significant;
                const isBonfSig = data.bonferroni?.is_significant_after_correction;
                return (
