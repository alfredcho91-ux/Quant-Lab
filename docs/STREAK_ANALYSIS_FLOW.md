...
sequenceDiagram
    ...
    participant DATA as utils/data_service.py
    participant STRAT as streak/simple_strategy.py, complex_strategy.py
...
### 2. 컨트롤러 (strategy/streak/__init__.py)
```
...
### 3. 공통 유틸리티
- **`strategy/context.py`**: `AnalysisContext` (분석 컨텍스트)
- **`strategy/streak/common.py`**: `load_data()`, `prepare_dataframe()`, 캐싱, `normalize_indices()` 등
- **`strategy/streak/statistics.py`**: `wilson_confidence_interval()`, `calculate_binomial_pvalue()` 등

### 4. Simple Mode 전략 (strategy/streak/simple_strategy.py)
```
...
```
↓
`strategy/streak/__init__.py::analyze_streak_pattern()`
...
```
↓
`strategy/context.py::AnalysisContext`
...
```
↓
`strategy/streak/common.py::load_data()`
  └─ `utils/data_service.py::load_csv_data()` 또는 `fetch_live_data()`
...
```
↓
`strategy/streak/common.py::prepare_dataframe()`
...
```
↓
`strategy/streak/simple_strategy.py::run_simple_analysis()`
...
- **공통 유틸리티**: `backend/strategy/context.py`, `backend/strategy/streak/common.py`, `backend/strategy/streak/statistics.py`
- **Simple 전략**: `backend/strategy/streak/simple_strategy.py`
- **Complex 전략**: `backend/strategy/streak/complex_strategy.py`
- **데이터 서비스**: `backend/utils/data_service.py`
