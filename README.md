...
| **🤖 AI 퀀트 연구소** | 자연어 기반 백테스트, 조건부 확률 분석, 파라미터 자동 최적화 | 자연어 프롬프트, Gemini API | 모든 타임프레임 |
...
### AI 퀀트 연구소 (AI Strategy Lab) 상세

| 기능 | 설명 | 입력 방식 | 주요 지표 |
|------|------|-----------|-----------|
| **자연어 백테스트** | 자연어 프롬프트를 분석하여 백테스트 파라미터로 변환 및 실행 | 자연어 입력 (예: "RSI 30 이하일 때 매수") | 승률, Profit Factor, MDD |
| **조건부 확률 분석** | 특정 지표 조건 달성 시 다음 봉의 상승/하락 확률 통계 분석 | 자연어 입력 (예: "3연속 음봉 후 양봉 뜰 확률") | p-value, Wilson CI, GATI Index |
| **파라미터 최적화** | OOS(Out-of-Sample) 데이터 기반 최적 파라미터 자동 탐색 | 자연어 입력 (예: "방금 전략 최적화해줘") | 최적 파라미터 조합, 성과 리포트 |

**주요 특징:**
- **보안 API 키 관리**: 프론트엔드가 아닌 백엔드 `.env`에서 Gemini API 키를 안전하게 관리
- **다국어 지원**: 한국어 및 영어 프롬프트 완벽 지원 (응답 언어 자동 맞춤)
- **모호성 해결**: 불명확한 프롬프트 입력 시 Clarifying 질문 리스트 반환
...
| **조건부 분해** | 패턴 완성 시점의 지표별 승률 | C1 (진입일) | RSI, 이격도, ATR% 구간별 승률 | 실시간 현재 시장 상황(Current) 매핑 및 하이라이팅, 조건 결합 히트맵 |
